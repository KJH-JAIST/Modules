...
LCC_MB = zeros(length(W_list),N);
SLCC_MB = zeros(length(W_list),N);
...
for k = 1:ITER    
    for j = 1:length(W_list)
        ...
        for i = 1:(length(Removelist)-1)
            ...
            if length(FandS) > 1
                LCC_MB(j,i) = LCC_MB(j,i) + FandS(1)/N;
                SLCC_MB(j,i) = SLCC_MB(j,i) + FandS(2)/N;
            else
                LCC_MB(j,i) = LCC_MB(j,i) + FandS(1)/N;
                SLCC_MB(j,i) = SLCC_MB(j,i) + 0;
            end
        end
